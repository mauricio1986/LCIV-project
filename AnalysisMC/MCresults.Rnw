...
\title{Monte Carlo results for: Instrumental Variables with Unobserved Heterogeneity: A Latent Class Approach}
...
This document presents the main results for the Monte Carlo experiment. 
...
sum_mc <- function(R){
  ...
  #rej <- rbind(mean(abs((gamma_q1 - gamma[1]) / sgamma_q1) > 1.96), 
  #             mean(abs((gamma_q2 - gamma[2]) / sgamma_q2) > 1.96), 
  #             mean(abs((delta_q1 - d1[1]) / sdelta_q1) > 1.96), 
  #             mean(abs((delta_q2 - d1[2]) / sdelta_q2) > 1.96), 
  #             mean(abs((rho_q1 - rho[1]) / srho_q1) > 1.96), 
  #             mean(abs((rho_q2 - rho[2]) / srho_q2) > 1.96), 
  #             mean(abs((share_q1 - prop[1]) / sshare_q1) > 1.96), 
  #             mean(abs((share_q2 - prop[2]) / sshare_q2) > 1.96))
  ...
  #out <- cbind(bias, rmse, rej)
  #colnames(out) <- c("Bias", "RMSE", "Cov")
  out <- cbind(bias, rmse)
...
rownames(table) <- c("$\\gamma_{1}$", 
                     ...
                     "$\\pi_{2}$")
table <- xtable(table)
caption(table) <- "Simulation results for LCIV parameters\\label{tab:table1}"
...
rownames(table) <- c("$\\gamma_{1}$", 
                     ...
                     "$\\pi_{2}$")
table <- xtable(table)
caption(table) <- "Simulation results for LCIV parameters\\label{tab:table3}"
digits(table) <- c(0, rep(4, 8))
align(table) <- "lcccccccc"
addtorow     <- list()
addtorow$pos <- list(-1)
